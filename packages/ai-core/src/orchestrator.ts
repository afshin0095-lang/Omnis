import type { AiExecutionRequest, ModelAdapter, ModelDescriptor, ModelInvocationResult } from "./types.js";
...
export class ModelOrchestrator {
  ...
  select(request: AiExecutionRequest): ModelDescriptor {
    ...
    if (request.modelRequirements?.maxLatencyMs !== undefined &&
        selected.averageLatencyMs > request.modelRequirements.maxLatencyMs) {
  ...
  async invoke(executionId: string, request: AiExecutionRequest, signal: AbortSignal): Promise<RoutedInvocation> {
    const candidates = this.fallback.candidates(request)
      ...
      .filter((candidate) => request.modelRequirements?.maxLatencyMs === undefined ||
    ...
    for (const candidate of candidates) {
      try {
        const result = await withRetry(async () => {
          attempts++;
          return candidate.adapter.invoke({
            executionId,
